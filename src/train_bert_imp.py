...
def pick_model_name(lang: str) -> str:
    mapping = {
        "eng": "bert-base-uncased",                       # English
        "deu": "bert-base-german-cased",                  # German (dbmdz)
        "ita": "dbmdz/bert-base-italian-uncased",         # Italian (dbmdz)  (use "...-cased" if you prefer)
        "spa": "dccuchile/bert-base-spanish-wwm-cased",   # Spanish (BETO wwm cased)
        "amh": "Davlan/afro-xlmr-base",                  # Amharic-friendly multilingual (AfroXLM-R)
        "urd": "ai4bharat/IndicBERTv2-MLM-only",                     # Urdu: strong multilingual fallback
        # keep the rest on mBERT cased
        "arb": "bert-base-multilingual-cased",
        "hau": "bert-base-multilingual-cased",
        "zho": "bert-base-multilingual-cased",
    }
    return mapping.get(lang, "bert-base-multilingual-cased")
...
def run_one_language(
    ...
    out_dir    = os.path.join("outputs_imp", lang)
...
def main():
    ...
    os.makedirs("outputs_imp", exist_ok=True)
    ...
    pd.DataFrame(summary).to_csv("outputs_imp/summary_metrics.csv", index=False)
    print("\n=== Summary ===")
    try:
